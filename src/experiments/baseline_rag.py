...
class BaselineResult:
    """Stores baseline experiment results with enhanced tracking."""
    query: str
...
class BaselineRAG:
    """
    Memory-efficient baseline RAG implementation with comprehensive tracking.
    """
    def __init__(
        self,
        ...
        self.experiment_name = experiment_name or f"baseline_rag_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.batch_size = batch_size
        self.enable_gc = enable_gc
        self.cache_dir = cache_dir or tempfile.mkdtemp(prefix="baseline_rag_")
        ...
        self.logger.info(
            f"Initialized BaselineRAG with {retriever_type} retriever "
    ...
    def setup_directories(self):
        ...
        try:
            ...
            self.results_dir = paths.BASELINE_RESULTS_DIR / timestamp
            self.models_dir = self.results_dir / "models"
            self.indices_dir = self.results_dir / "indices"
            self.temp_dir = self.results_dir / "temp"
            
            for dir_path in [self.results_dir, self.models_dir, self.indices_dir, self.temp_dir]:
    ...
    def initialize_components(
        ...
        """Initialize components with index validation."""
        try:
    ...
    def _save_batch_results(self, results: List[BaselineResult], batch_idx: int):
    ...
    def run_experiment(
        ...
        """
        Run baseline RAG experiment with memory-efficient batch processing.
        """
        try:
            # Initialize components
    ...
    def _process_query(
        ...
        """Process a single query with memory optimization."""
        try:
            ...
            if gold_doc_idx >= 0:
                gold_doc_idx = self.id_mapper.get_index_mapping(
                    gold_doc_idx,
                    'dataset_idx_to_corpus') or gold_doc_idx
            ...
            if inject_random and random_ratio > 0:
                num_random = int(len(retrieved_docs) * random_ratio)
                if num_random > 0:
                    ...
                    final_docs = retrieved_docs + random_docs
    ...
    def _prepare_context(self, documents: List[RetrievalResult]) -> str:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run baseline RAG experiment"
    )
    parser.add_argument(
        '--output_dir',
        type=str,
    ...
    try:
        ...
        experiment.logger.info(f"Baseline experiment completed in {duration}")
